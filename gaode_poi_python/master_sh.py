...
poi_grid_search_url = 'https://restapi.amap.com/v3/place/polygon'
...
def write_to_csv(poilist, citycode, classfield, coord):
    ...
    if len(poilist) == 0:
        print("处理完成，当前city-code:" + str(citycode), ", class-field为：", str(classfield) + "，数据为空，，，结束.......")
        return None, None
    ...
    for i in range(len(poilist)):
        ...
        if str(type) is not None and str(type) != '':
    ...
    data_csv['name'], data_csv['lon'], data_csv['lat'], data_csv['address'], data_csv['pname'], data_csv['cityname'], data_csv['adname'], data_csv['business_area'], data_csv['type'], data_csv['typecode'], data_csv['id'], data_csv['type1'], data_csv['type2'], data_csv['type3'], data_csv['type4'] = names, lons, lats, addresss, pnames, citynames, adnames, business_areas, types, typecodes, ids, type_1s, type_2s, type_3s, type_4s

    df = pd.DataFrame(data_csv)

    file_name = 'POI_' + citycode + "_" + classfield + ".csv"
    df.to_csv(r'data' + os.sep + 'POI_' + citycode + "_" + classfield + ".csv", index=False, encoding='utf_8_sig')
    print('写入成功')

    # folder_name = 'poi-' + citycode + "_" + classfield
...
def getpoi_page(grids, types, page, key):
    ...
    req_url = poi_grid_search_url + "?key=" + key + '&extensions=all&types=' + quote(
        types) + '&polygon=' + polygon + '&offset=25' + '&page=' + str(page) + '&output=json'
    print('请求url：', req_url)
...
def get_drids(min_lng, max_lat, max_lng, min_lat, keyword, key, grid_split_distance, all_grids):
    grids_lib = city_grid.generate_grids(min_lng, max_lat, max_lng, min_lat, grid_split_distance)
    ...
    for grid in grids_lib:
        one_grid_data = getpoi_page(grid, keyword, 1, key)
        data = json.loads(one_grid_data)
        print(data)

        while int(data['count']) > 890:
            get_drids(grid[0], grid[1], grid[2], grid[3], keyword, key, grid_split_distance / 2, all_grids)
...
def get_data(city, keyword, coord):
    ...
    for grid in grids_lib:
        ...
        one_grid_data = getpois(grid, keyword)

        print('=========== 当前矩形范围：', grid, '总共：',
              str(len(one_grid_data)) + "条数据..............")

        all_data.extend(one_grid_data)
    ...
    # trans_point_to_shp(file_folder, file_name, 0, 1, grid_split_distance, keyword)
